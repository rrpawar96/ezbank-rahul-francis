...
public class SavingsAccountDomainServiceJpa implements SavingsAccountDomainService {
	...
	public SavingsAccountTransaction handleWithdrawal(final SavingsAccount account, final DateTimeFormatter fmt,
		...
		if (transactionBooleanValues.isRegularTransaction() && !account.allowWithdrawal()) {
			throw new DepositAccountTransactionNotAllowedException(account.getId(), "withdraw",
		...
		final SavingsAccountTransactionDTO transactionDTO = new SavingsAccountTransactionDTO(fmt, transactionDate,
				transactionAmount, paymentDetail, new Date(), user, accountType);
	...
	/*	if (parentEvent != null) {
			...
			for (int i = lastElement; i >= 0; i--) {
				// fetch charge out of each transaction of account in reverse
				// order
				currentTransaction = chargeTransactions.get(i);
				charge = this.savingsAccountChargePaidByRepository
						.findOneBySavingsAccountTransactionId(currentTransaction.getId());
				// not necessary each transaction might be a charge.. hence if
				// charge found, verify its the one we need
				if (charge != null) {
					if (charge.getSavingsAccountCharge().getCharge().getChargeTimeType() == chargeTimeType.getValue()) {
						InterswitchSubEvents subEvent2 = InterswitchSubEvents
								.getInstance(InterswitchEventType.CHARGE.getValue(), parentEvent, currentTransaction);
	...
	public SavingsAccountTransaction handleLoanDeposit(final SavingsAccount account, final DateTimeFormatter fmt,
		...
		return handleDeposit(account, fmt, transactionDate, transactionAmount, paymentDetail, isAccountTransfer,
				isRegularTransaction, savingsAccountTransactionType, isLoanDisbursement, null);
	}
	...
	public SavingsAccountTransaction handleRetailDeposit(final SavingsAccount account, final DateTimeFormatter fmt,
		...
		final SavingsAccountTransactionType savingsAccountTransactionType = SavingsAccountTransactionType.DEPOSIT;
		return handleDeposit(account, fmt, transactionDate, transactionAmount, paymentDetail, isAccountTransfer,
				isRegularTransaction, savingsAccountTransactionType, false, transactionExternalId);
	}
	...
	public SavingsAccountTransaction handleInterswitchDeposit(final SavingsAccount account, final DateTimeFormatter fmt,
		...
		return handleDeposit(account, fmt, transactionDate, transactionAmount, paymentDetail, isAccountTransfer,
				isRegularTransaction, savingsAccountTransactionType, false, null);
	}

	/*
	 * handleDeposit was overloaded to handle new transaction type
	 * loandisbursement
	 */
	...
	private SavingsAccountTransaction handleDeposit(final SavingsAccount account, final DateTimeFormatter fmt,
		...
		if (isRegularTransaction && !account.allowDeposit()) {
			throw new DepositAccountTransactionNotAllowedException(account.getId(), "deposit",
					account.depositAccountType());
		}
